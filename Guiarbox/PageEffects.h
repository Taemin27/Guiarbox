#ifndef PAGE_EFFECTS_H
#define PAGE_EFFECTS_H

// 'Effects', 160x80px
const unsigned char effectsHome [] = {
...
int ef_compressor_level = 5;
int ef_compressor_threshold = 5;
int ef_compressor_release = 5;
...
void ef_drawCompressor() {
  ...
  if (ef_compressor_on) {
    ...
    compressorOnOffMixer.gain(0, 0);
    compressorOnOffMixer.gain(1, 1);

    compressor.setLevel(((ef_compressor_level - 1) / 9.0f) * 2.0f);                   // 0.0 to 2.0
    compressor.setThreshold(0.1f + ((ef_compressor_threshold - 1) / 9.0f) * 0.4f);
    compressor.setRelease(1.0f - expf(-1.0f / (10.0f * powf(50.0f, ((ef_compressor_release - 1) / 9.0f)) * 44.1f)));
